import React, { useEffect, useMemo, useRef, useState } from "https://esm.sh/react@18.2.0?target=es2019";
...
function normalizeVersion(raw, fallbackTitle, forceType) {
    ...
    return {
        ...
        title: typeof fallbackTitle === "string" && fallbackTitle.trim().length > 0 ? fallbackTitle : "Untitled",
        content: typeof source.content === "string" ? source.content : "",
...
export function App() {
    ...
    const versions = activeId && Array.isArray(versionsByNote[activeId]) ? versionsByNote[activeId] : [];
    const selectedVersion = versions.find((entry) => entry.id === selectedVersionId) || null;

    async function fetchAndStoreVersions(noteId, fallbackTitle, markLatestAsAi) {
        try {
            const response = await getNoteVersions(noteId);
            const rows = response && Array.isArray(response.versions) ? response.versions : [];

            const mapped = rows.map((row) => normalizeVersion(row, fallbackTitle, "user"));
            ...
            setErrorMessage(error && error.message ? error.message : "Failed to load timeline.");
        }
    }
    ...
    useEffect(() => {
        ...
        async function bootstrap() {
            ...
            try {
                ...
                clearAuthSession();
                setSession(null);
                setAuthError(error && error.message ? error.message : "Session expired. Please login again.");
            }
        }
    ...
    useEffect(() => {
        ...
        if (synced && synced.title === activeNote.title && synced.content === activeNote.content) {
            return;
        }
        ...
        saveTimerRef.current = window.setTimeout(async () => {
            try {
                ...
                setErrorMessage(error && error.message ? error.message : "Failed to sync note.");
            }
        }, 650);
    ...
    async function handleSignup(email, password) {
        try {
            ...
            const maybeAccessToken = signupResponse && signupResponse.session ? signupResponse.session.access_token : null;
            ...
            setAuthError("Signup successful. Please verify email if required, then login.");
        } catch (error) {
    ...
    async function handleCreateNote() {
        try {
            ...
            setErrorMessage(error && error.message ? error.message : "Failed to create note.");
        }
    }
    ...
    async function handleRestoreVersion(versionId) {
        ...
        try {
            ...
            setErrorMessage(error && error.message ? error.message : "Failed to restore version.");
        }
    }
    ...
    async function handleAiRewrite() {
        ...
        try {
            ...
            if (result.status !== "done") {
                throw new Error(typeof result.output === "string" && result.output ? result.output : "Rewrite job failed.");
            }
            ...
            setErrorMessage(error && error.message ? error.message : "AI rewrite failed.");
        } finally {
    ...
    return React.createElement(
        "div",
        {
            className: "relative h-screen overflow-hidden bg-[#0f0f0f] font-sans text-zinc-200 antialiased",
        },
        React.createElement("div", {
            ...
            React.createElement("main", { className: "relative flex min-w-0 flex-col border-r border-zinc-800/70" },
                React.createElement("header", {
                    className: "flex h-12 items-center gap-2 border-b border-zinc-800/70 px-3 text-sm md:px-6",
                },
                React.createElement("button", {
                    className: "inline-flex h-8 w-8 items-center justify-center rounded-md text-zinc-400 transition hover:bg-zinc-800/60 hover:text-zinc-200 focus:outline-none",
                    type: "button",
                ...
                errorMessage
                    ? React.createElement("div", {
                        className: "border-b border-rose-500/30 bg-rose-500/10 px-4 py-2 text-xs text-rose-200",
                    }, errorMessage)
                    : null,
